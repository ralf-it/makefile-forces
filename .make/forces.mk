...
%:
	...
	if [ "$(ARGV0)" != "$(MAKECMDGOALS)" ]; then # ! NOTE: disable parallelism, change behaviour of make TGT1 TG2 to make TGT1 ARGS...
		if [ "$(SILENT)" == "false" ]; then
			echo "ARGV0, ARGV != MAKECMDGOALS: $(ARGV0), $(ARGV) != $(MAKECMDGOALS)"
		fi
		exit 0
	fi
	@:
...
MAKEFILE_FORCES_VERSION ?= $(file < VERSION)
...
makefile-forces-pip-update-install: ## {<<FORCES>>} update and install makefile-forces from github via pip
	$(M) $@+INFO-B
	set +x
	echo pip install git+$(MAKEFILE_FORCES_GIT).git@v$(MAKEFILE_FORCES_VERSION) --verbose --force
